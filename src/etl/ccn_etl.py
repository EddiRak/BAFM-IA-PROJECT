...
RAW_DATA_PATH = os.environ.get("RAW_DATA_CCN", os.path.join(BASE_PATH, "raw_data/CCN"))
PROCESSED_DATA_PATH = os.environ.get("PROCESSED_DATA_CCN", os.path.join(BASE_PATH, "processed_data/ETL/CCN"))
...
# ------------------ Lecture fichier ------------------
def _read_ccn_file(file_path: str, columns: List[str]) -> pd.DataFrame:
    try:
        df = pd.read_csv(
            file_path,
            sep=SEP_CCN,
        ...
        logger.error(f"[CCN] Lecture échouée: {os.path.basename(file_path)} -> {e}")
...
def main():
    ...
    if not files:
        logger.error(f"[CCN] Aucun fichier trouvé dans {RAW_DATA_PATH} ({PATTERN})")
        return
    ...
    with Client(n_workers=n_workers) as client:
        ...
        futures = [client.submit(_read_ccn_file, fp, ccn_cols) for fp in files]
        for fut in futures:
            df = fut.result()
    ...
    if results:
        ...
        out_path = os.path.join(PROCESSED_DATA_PATH, "ccn.parquet")
        combined.to_parquet(out_path, index=False)
        logger.info(f"• CCN : {len(combined):,} enregistrements")
    else:
        logger.warning("[CCN] Aucune donnée consolidée")
